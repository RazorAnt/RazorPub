...
public class FeedHandler : IHttpHandler
{
    public void ProcessRequest(HttpContext context)
    {
        ...
        {
            Title = new TextSyndicationContent(Site.Title),
    ...
    {
        foreach (Post p in Site.GetPosts(10))
        {
            var item = new SyndicationItem(p.Title, p.GetHtmlContent(), p.AbsoluteUrl, p.AbsoluteUrl.ToString(), p.LastModified)
                       {
                           PublishDate = p.PubDate
                       };

            if (!string.IsNullOrWhiteSpace(p.Description))
                item.Summary = new TextSyndicationContent(p.Description);
